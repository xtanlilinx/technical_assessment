...
CONFIG = {
    ...
    "max_audio_length": 120, # Long audios might be computationally expensive > to prevent OOMs errors, skip them and log as [TOO LONG]
    ...
    for path in file_batch_paths:
        ...
        try:
            ...
            if audio.info.length > CONFIG["max_audio_length"]:
                batch_results.append({"filename": rel_path, "generated_text": "[TOO LONG]", "duration": f"{audio.info.length:.1f}"})
                continue
        except Exception:
            batch_results.append({"filename": rel_path, "generated_text": "[CORRUPT]", "duration": "0.0"})
            continue
        ...
        try:
            ...
            for i, result in enumerate(api_outputs):
                ...
                batch_results.append({
                    ...
                    "duration": f"{result.get('duration', 0.0):.1f}"
                })
        except Exception as e:
            print(f"Batch Request Failed: {e}")
            # Log failed files as errors
            for item in files_to_send:
                batch_results.append({"filename": f"{CONFIG['subfolder']}/{item[1][0]}", "generated_text": "[API ERROR]", "duration": "0.0"})
        finally:
...
if __name__ == "__main__":
    ...
    if TEMP_PATH.exists():
        ...
        if 'duration' in master_df.columns:
            master_df = master_df.drop(columns=['duration'])
